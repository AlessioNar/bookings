...
func TestRepository_PostReservation(t *testing.T) {
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler return wrong response code for invalid room id: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusSeeOther {
		t.Errorf("PostReservation handler return wrong response code for invalid data: got %d, wanted %d", rr.Code, http.StatusSeeOther)
	}
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler failed when trying to insert reservation in DB: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler failed when trying to insert restriction in DB: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("Post Availability when no rooms available gave wrong response code: got %d wanted %d", rr.Code, http.StatusSeeOther)
	}
	...
	if rr.Code != http.StatusOK {
		t.Errorf("Post Availability when no rooms available gave wrong response code: got %d wanted %d", rr.Code, http.StatusOK)
	}
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Post Availability with empty post body gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Fourth case: start_date in wrong format
	reqBody = "start_date=invalid"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2040-01-02")
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Post Availability with invalid start date gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Fifth case: end_date in wrong format
	reqBody = "start_date=2040-01-02"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=invalid")
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Post Availability with invalid end date gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Post Availability when database query fails gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
...
func TestRepository_AvailabilityJSON(t *testing.T) {
	...
	// 1st test Rooms are not available
	reqBody := "start_date=2050-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-02")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "room_id=1")
...
func TestRepository_ReservationSummary(t *testing.T) {
	// case 1 reservation is in session
	...
	if rr.Code != http.StatusOK {
		t.Errorf("ReservationSummary handler gave wrong response code: got %d wanted %d", rr.Code, http.StatusOK)
	}

	// Case 2 Reservation is not in session
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("ReservationSummary handler gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
}

func TestRepository_ChooseRoom(t *testing.T) {
	// First case, reservation is in session
	reservation := models.Reservation{
		RoomID: 1,
		Room: models.Room{
			ID:       1,
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("ChooseRoom handler gave wrong response code: got %d wanted %d", rr.Code, http.StatusSeeOther)
	}

	// Second case, reservation is not in session \
	req, _ = http.NewRequest("GET", "/choose-room/1", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("ChooseRoom handler gave wrong response code: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// Third case, missing url parameter or malformed url parameter
