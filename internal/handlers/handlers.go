...
// PostAvailability handles the posting of the Reservation form
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	// Get start and end date and convert them in correct format
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		// Loop through entire map. If we have an entry
		//that does not exist in our posted data and
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			// ok will be false if the value is not in the map
			if val, ok := curMap[name]; ok {
				// only pay attention to values which are greater than 0 and they are not in the form post
				if val > 0 {
